...
function useInstanceBeforeDimensions(instance) {
  ...
  var handleColumn = function handleColumn(column, parentVisible) {
    column.isVisible = typeof column.isVisible !== 'undefined' ? column.isVisible : parentVisible && !hiddenColumns.includes(column.id);
...
var useTable = function useTable(props) {
  ...
  var columns = React.useMemo(function () {
    return decorateColumnTree(reduceHooks(getColumnsHooks(), userColumns, getInstance()), defaultColumn);
  }, [defaultColumn, getColumnsHooks, getInstance, userColumns].concat(reduceHooks(getColumnsDepsHooks(), [], getInstance())));
  ...
  flatColumns = React.useMemo(function () {
    return reduceHooks(flatColumnsHooks(), flatColumns, getInstance());
  }, [flatColumns, flatColumnsHooks, getInstance].concat(reduceHooks(flatColumnsDepsHooks(), [], getInstance())));
  ...
  }, [defaultColumn, flatColumns, getHeaderGroups, getInstance].concat(reduceHooks(getHeaderGroupsDeps(), [], getInstance())));
  ...
  getInstance().prepareRow = React.useCallback(function (row) {
    ...
    row.cells = reduceHooks(cellsHooks(), row.allCells, getInstance()); // need to apply any row specific hooks (useExpanded requires this)
...
function useFlexLayout(hooks) {
  hooks.getTableBodyProps.push(function (props, instance) {
    return [props, {
      style: {
...
export { actions, defaultColumn, defaultGroupByFn, defaultOrderByFn, ensurePluginOrder, flexRender, functionalUpdate, loopHooks, makePropGetter, makeRenderer, reduceHooks, safeUseLayoutEffect, useAbsoluteLayout, useAsyncDebounce, useBlockLayout, useColumnOrder, useConsumeHookGetter, useExpanded, useFilters, useFlexLayout, useGetLatest, useGroupBy, useMountedLayoutEffect, usePagination, useResizeColumns, useRowSelect, useRowState, useSortBy, useTable };
